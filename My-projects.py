...
with st.container(width="stretch", horizontal_alignment="center", gap="large"):
    ...
    with col1:
        ...
        with cent:
            st.link_button("Machine Learning Algorithms", "https://github.com/aakarroy/Machine-Learning-Algorithms", use_container_width=True)
    ...
    with col2:
        ...
        _, cent, _ = st.columns([0.2, 1, 0.2])
        with cent:
            st.link_button("Python Projects", "https://github.com/aakarroy/Nexus-Of-Knowledge-Library-Management-System", use_container_width=True)
    ...
    with col3:
        ...
        _, cent, _ = st.columns([0.1, 1, 0.1])
        with cent:
            st.link_button("Deep Learning Models", "https://github.com/aakarroy/BCI-CNN-LSTM", use_container_width=True)
    ...
    """, unsafe_allow_html=True)
        _, cent, _ = st.columns([0.2, 1, 0.2])
        with cent:
            st.link_button("SAHI-BIN", "https://github.com/aakarroy/SahiBin-AI", use_container_width=True)
            
    st.divider()
    
    col5, col6 = st.columns([1,1], vertical_alignment="center", gap="large")
    with col5:
        st.markdown("""
        <div style="
            text-align: center; 
            font-family: 'Arial Black', Gadget, sans-serif;
            white-space: nowrap;">
            <span style="color: #00b4d8; font-size: 60px; font-weight: 900; letter-spacing: -3px;">Tooth</span><span style="color: #ff70a6; font-size: 60px; font-weight: 900; letter-spacing: -3px;">Fairy</span>
        </div>
        """, unsafe_allow_html=True)
        _, cent, _ = st.columns([0.2, 1, 0.2])
        with cent:
            st.link_button("Tooth Fairy", "https://github.com/aakarroy/ToothFairy2", use_container_width=True)